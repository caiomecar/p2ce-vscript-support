...
fn param_tag_name(
    ...
    let Type::Function(Some(id)) = finished_file.get(*symbol).typ else {
        return None;
    };
    ...
    match token.kind() {
        ...
        SyntaxKind::ColonColon => {
            ...
            Some(ContextCompletions::FromQualifiedName {
                typ: Some(finished_file.get(*symbol).typ),
            })
        }
        SyntaxKind::String => {
            if !touching {
            ...
            if !matches!(trigger_char, Some("\"") | None) {
                return None;
            }
        ...
        SyntaxKind::DocAsteriskSlash | SyntaxKind::DocSlashAsteriskAsterisk => {
            ...
            Some(ContextCompletions::DocAutoGenerated {
                typ: finished_file.get(*symbol).typ,
                replace_range: range,
            })
        }
...
fn completions_doc_type(offset: TextSize, finished_file: &FinishedFile<'_>) -> Vec<CompletionItem> {
    ...
    tags.into_iter()
        ...
        .chain(symbols.into_iter().filter_map(|(name, id)| {
            if !matches!(finished_file.get(id).typ, Type::Class(_)) {
                return None;
            }
            Some(CompletionItem {
